	...
	for_each_set_bit_from(i, drv->tcs_in_use, tcs->offset + tcs->num_tcs) {
		...
		for_each_set_bit(j, &curr_enabled, tcs->ncpt) {
			...
			for (k = 0; k < msg->num_cmds; k++) {
				/*
				 * Each RPMh VREG accelerator resource has 3 or 4 contiguous
				 * 4-byte aligned addresses associated with it. Ignore the
				 * offset to check for in-flight VREG requests.
				 */
