	...
	if (!rc) {
		...
		if (glink_crash_num > 20) {
			...
			if (bcdev->glink_crash_count > 3) {
				...
				schedule_delayed_work(&bcdev->glink_crash_num_work,
						      msecs_to_jiffies(GLINK_CRASH_RESET_HTIME_MS));
			} else {
				schedule_delayed_work(&bcdev->glink_crash_num_work,
						      msecs_to_jiffies(GLINK_CRASH_RESET_LTIME_MS));
			}
			return 0;
		} else {
...
static void battery_chg_update_usb_type_work(struct work_struct *work)
{
	...
	for (i = 0; i < NUM_USB_PORTS; i++) {
		...
		/* Honor power-absent reporting override */
...
static int battery_chg_init_psy(struct battery_chg_dev *bcdev)
{
	...
	bcdev->psy_list[PSY_TYPE_BATTERY].psy =
		devm_power_supply_register(bcdev->dev, &batt_psy_desc, &psy_cfg);
...
static int battery_chg_parse_dt(struct battery_chg_dev *bcdev)
{
	...
	if (rc <= 0) {
		...
		if (pst->prop[BATT_CHG_CTRL_LIM_MAX] -
		    (bcdev->thermal_fcc_step * len) < 500000)
			len = len - 1;
	} else {
	...
	bcdev->shutdown_delay_en =
		of_property_read_bool(node, "mi,support-shutdown-delay");
	bcdev->fast_update_temp =
		of_property_read_bool(node, "mi,support-fast-update-timer");
