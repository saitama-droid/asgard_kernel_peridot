...
{
	struct vm_area_struct lpad;

	if (!is_pgsize_migration_enabled() || !(vma->vm_flags & VM_PAD_MASK))
		return;

	init_pad_vma(vma, &lpad);

	/* The pad VMA should be anonymous. */
	BUG_ON(lpad.vm_file);

	/* The pad VMA should be PROT_NONE. */
	BUG_ON(lpad.vm_flags & (VM_READ|VM_WRITE|VM_EXEC));

	/* The pad VMA itself cannot have padding; infinite recursion */
	BUG_ON(lpad.vm_flags & VM_PAD_MASK);

	if (smaps)
		((show_pad_smaps_fn)func)(m, &lpad);
	else
		((show_pad_maps_fn)func)(m, &lpad);
}
